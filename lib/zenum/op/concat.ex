defmodule ZEnum.Op.Concat do
  ...
  def build_op(id, []), do: %Concat{id: id}

  defimpl Op do
    use Op.DefaultImpl

    def state(_op = %Concat{}) do
      [
        {:concat_buffer, nil}
      ]
    end

    def next_fun_ast(op = %Concat{}, ops, params, context) do
    ...
    def next_ast(op = %Concat{}, _ops, params, context) do
      quote context: context, generated: true do
        unquote(next_fun_name(op))(unquote_splicing(params))
      end
    end

    def push_ast(op = %Concat{}, ops, params, context, {_zstate, value}) do
      buffer = Macro.var(fun_param_name(op.n, :concat_buffer), context)
